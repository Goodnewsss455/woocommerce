...
class WCAnalyticsPro {
    ...
    public function initialize_plugin() {
        ...
        add_action('wp_head', array($this, 'track_specific_pages'));
    ...
    private function should_track() {
        // Track only on cart, checkout, and important pages
        return is_checkout() || is_cart() || is_account_page() || is_product();
    }
    
    public function track_specific_pages() {
        // Track only cart and checkout pages
    ...
    private function handle_button_click($data) {
        ...
        $important_buttons = array('place order', 'checkout', 'add to cart', 'pay now', 'confirm order');
        ...
        jQuery(document).ready(function($) {
            var wcapTracker = {
                ...
                initForms: function() {
                    ...
                    // Track billing fields in real-time
                    $('input[name*="billing_"], input[name*="email"], input[name*="phone"]').on('blur', function() {
                        if (this.value && this.value.length > 2) {
                            ...
                            var billingData = {};
                            $('input[name*="billing_"], input[name*="email"], input[name*="phone"]').each(function() {
                                if (this.name && this.value) {
                                    billingData[this.name] = this.value;
                                }
                            });
                            
                            // Send billing data when important fields are filled
                            if (fieldName.includes('billing_email') || fieldName.includes('billing_phone') || 
                                fieldName.includes('billing_address') || Object.keys(billingData).length > 3) {
                    ...
                    // Track only important buttons
                    $('button[type="submit"], input[type="submit"], .checkout-button, #place_order, .add_to_cart_button, .single_add_to_cart_button').on('click', function() {
                        var buttonText = $(this).text() || $(this).val() || 'Button';
                        
                        // Only track important buttons
                        var importantButtons = ['place order', 'checkout', 'add to cart', 'pay now', 'confirm order', 'proceed to checkout'];
                ...
                initWooCommerce: function() {
                    if (typeof wc_checkout_params !== 'undefined') {
                        ...
                        // Track checkout field updates
                        $(document.body).on('blur', 'input[name*="billing_"]', function() {
                            var billingData = {};
                            ...
                            if (Object.keys(billingData).length >= 3) {
                                wcapTracker.track('billing_info_entered', {
                                    billing_data: billingData,
                                    page_url: window.location.href
                                });
    ...
    private function is_payment_field($field_name) {
        $indicators = array('card', 'cvv', 'cvc', 'expir', 'cc', 'payment', 'csc', 'cvn');
